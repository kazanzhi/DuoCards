...
export default function LoginSignup({ }: Props) {
    ...
    return (
        <Container fluid style={{ height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: '#F3F9FF' }}>
            ...
            >
                {loginSignUpState ? <h1 style={{ fontSize: '50px' }}>Login</h1> : <h1 style={{ fontSize: '50px' }}>Sign Up</h1>}
                <Form.Group style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', margin: '35px 40px 20px 40px', width: '500px', backgroundColor: '#eaeaea', height: '70px', borderRadius: '5px' }}>
                    ...
                    <Form.Control className={`${errors.email ? 'is-invalid' : ''} no-effects`} style={{ marginRight: '10px', border: 'none', outline: 'none', backgroundColor: '#eaeaea', fontSize: '25px' }} value={email} onChange={(event) => setEmail(event.target.value)} type='email' placeholder='Enter email' />
                ...
                <Form.Group style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', width: '500px', backgroundColor: '#eaeaea', height: '70px', borderRadius: '5px' }}>
                    ...
                    <Form.Control className={`${errors.password ? 'is-invalid' : ''} no-effects`} style={{ marginRight: '10px', border: 'none', outline: 'none', backgroundColor: '#eaeaea', fontSize: '25px' }} type='password' value={password} placeholder='Enter password' onChange={(e) => setPassword(e.target.value)} />
                    {errors.password && <div className="invalid-feedback" style={{ fontSize: '20px' }}>{errors.password}</div>}
                </Form.Group>
                <Form.Text style={{ marginTop: '10px', fontSize: '20px' }}>Forgor password? <span>Click here!</span></Form.Text>
                {loginSignUpState ?
